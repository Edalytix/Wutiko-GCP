...
class UserAPIController extends Controller
{
    ...
    function update_location(Request $request)
    {
        ...
        $userid = $data["user_id"];
        $data_array = array();
        $data_array['longitude'] = $data["longitude"];
        $data_array['latitude'] = $data["latitude"];
        //$insert_address =  DB::select("UPDATE addresses SET latitude='$lat',longitude='$long' WHERE user_id = '$userid'");
        ///dd("UPDATE addresses SET latitude='$lat',longitude='$long' WHERE user_id = '$userid'");
        $location = DB::table('addresses')->where("user_id",$userid)->update($data_array);
        if($location == 0)
        {
            return array(
                "status" => 0,
                "message" => "invalid userid or unable to update same coordinates"
            );
        }
        else
        {
